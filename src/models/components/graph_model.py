...
class CustomResNet(models.ResNet):
    ...
    def _forward_impl(self, x):
        # This part mimics the original ResNet's forward pass up to the last convolutional layer
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        # Bypassing the avgpool and fc layers
        return x
...
# Now, the custom_resnet18 model will output the feature map directly from the convolutional layers
# Here's a dummy input tensor for demonstration
...
class HGCN(nn.Module):
    ...
    def __init__(self,k=9,act='gelu',norm='batch',bias=True,dropout=0.0,dilation=True,epsilon=0.2,drop_path=0.1,size ='m',
        ...
        original_conv1_weight = original_resnet18.conv1.weight.data
        averaged_weights = original_conv1_weight.mean(dim=1, keepdim=True)

        self.custom_resnet18 = CustomResNet(models.resnet.BasicBlock, [2, 2, 2, 2])
        self.custom_resnet18.conv1.weight.data = averaged_weights
        state_dict = original_resnet18.state_dict()
        state_dict['conv1.weight'] = averaged_weights
        self.custom_resnet18.load_state_dict(state_dict, strict=False)
...
# Remove or replace the fully connected layer
        ...
        self.projection_q2l = Seq(nn.Conv2d(self.channels[-1], 1024, 1, bias=True),
                              nn.BatchNorm2d(1024),
                              act_layer(act),
                              nn.Dropout(self.dropout))
                              
        self.query_fc = GroupWiseLinear(num_class, 1024, bias=True)
        self.query_pos = nn.Parameter(torch.zeros(1, 1024, num_class))
        self.key_pos = nn.Parameter(torch.zeros(1, 1024, freq_num//32 * time_num//32))
        self.transformer = Transformer(1024, depth=1, heads=1,dropout=0.1,act='gelu')
    ...
    def forward(self,inputs):
        ...
        if self.q2label: 
            query_input = self.query_embed.weight
             # query input shape torch.Size([200, 640])

            x_mod = self.projection_q2l(x)
             # x_mod shape torch.Size([8, 1024, 4, 32])

            query_input = self.label_proj(query_input.unsqueeze(0).unsqueeze(-1).permute(0,2,1,3)) # query_input shape torch.Size([1, 1024, 200, 1])
            
            
            query_input = query_input + self.query_pos.unsqueeze(-1)
             # query input shape torch.Size([1, 1024, 200, 1])
            x_mod_pos = x_mod.reshape(B,1024,-1,1) + self.key_pos.unsqueeze(-1)
            x_mod_pos = x_mod_pos.squeeze(-1).transpose(1,2)
            x = x.reshape(B,1024,-1,1).squeeze(-1).transpose(1,2)
             # x_mod shape torch.Size([B, 200, 1024])
             # x shape torch.Size([B, 200, 1024])
            query_input = query_input.repeat(B,1,1,1).squeeze(-1).transpose(1,2)
             # query input shape torch.Size([B, 200, 1024]
            tgt = torch.zeros_like(query_input,device = x.device)
            
            label_out = self.transformer(tgt,x_mod_pos,x_mod.reshape(B,1024,-1,1).squeeze(-1).transpose(1,2)) # label_out shape torch.Size([B, 200, 1024])
            
            preds = self.query_fc(label_out)
